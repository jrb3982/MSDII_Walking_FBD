...
void ADC0_InitSWTriggerCh6(void)
{
	...
	// REF_A->CTL0
  while ((REF_A->CTL0 & 0x10000) == 0x10000);       
  ...
  // 32 clocks
  ADC14->CTL0 &= ~(BITF | BITE | BITB | BITA);
  ADC14->CTL0 |= (BITD | BITC | BIT9 | BIT8);
  // SHM pulse-mode
  ADC14->CTL0 |= BIT(26);
  ...
  // 1-0   ADC14PWRMD ADC power modes        00b = Regular power mode
	...
	// ADC14->CTL1
  // ADC14MEM0
  ADC14->CTL1 &= ~(BIT(20) | BIT(19) | BIT(18) | BIT(17) | BIT(16));
  // 14-bit
  ADC14->CTL1 |= (BIT5 | BIT4);
  // ref on
  ADC14->CTL1 &= ~(BIT2);
  // regular power          
  ADC14->CTL1 &= ~(BIT1 | BIT0);
